...
function getDepotItemCount(itemId, tier)
    for _, data in pairs(depotLockerItems) do
        -- Current C++ binding exposes named fields.
        local depotItemId = data.itemId or data[1]
        local depotTier = data.tier
        local depotCount = data.count

        if depotCount ~= nil then
            if depotItemId == itemId and (depotTier or 0) == (tier or 0) then
                return depotCount
            end
        else
...
function configureList()
    marketItems = {}
    -- Protocol 15.30 also uses category 32.
    for c = 1, 32 do
        marketItems[c] = {}
    end
    ...
    -- Some protobuf appearance catalogs contain marketData but do not expose
    -- ThingAttrMarket through hasAttr(). In that case the optimized lookup is
    -- empty, so fall back to all item types and validate marketData below.
    if not types or not next(types) then
        types = g_things.getThingTypes(ThingCategoryItem) or {}
    end
    ...
    for _, itemType in pairs(types) do
        if itemType:getId() ~= 49870 and itemType:getId() ~= 14258 then
            ...
            if item then
                ...
                if marketData and marketData.category and marketData.name and marketData.name ~= "" then
                    local displayId = tonumber(marketData.showAs) or 0
                    if displayId <= 0 then
                        displayId = itemType:getId()
                    end
                    item:setId(displayId)
                    local marketItem = {
                        displayItem = item,
                        thingType = itemType,
...
-- Main Window
function onMarketEnter(offerCount, items, balance, vocation)
    configureList()
...
function onMarketBrowse(intOffers, nameOffers, browseState, itemTier)
    -- Modern market packets already provide every offer in intOffers.  Build
    -- the own-offers/history lists directly instead of depending on the
    -- per-offer callback having populated temporary tables first.  With two
    -- market layouts available, callback ordering is not guaranteed.
    if browseState == 1 or browseState == 2 then
        local buyOffersData = {}
        local sellOffersData = {}
        ...
        for i, raw in ipairs(intOffers or {}) do
            local offer = {
                ...
                tier = raw[9] or 0,
                itemTier = raw[9] or 0,
                holder = nameOffers and nameOffers[i] or ''
            }

            if offer.action == 0 then
                table.insert(buyOffersData, offer)
            else
                table.insert(sellOffersData, offer)
            end
        end
...
function onMarketDetail(itemID, tier, details, purchase, sale)
    ...
    for i, str in pairs(details) do
        local text = type(str) == 'table' and (str.desc or str[1] or '') or str
        if text and #text > 0 then
            local widget = g_ui.createWidget('DatailsLabel', marketWindow.contentPanel.detailsMarket.detailsList)
            widget:setText((MarketDetailNames[i] or ("Detail " .. i .. ": ")) .. text)
        end
    end
    ...
    if #purchase > 0 then
        ...
        local numTransactions = stats.numTransactions or stats[3] or 0
        local totalPrice = stats.totalPrice or stats[4] or 0
        local highestPrice = stats.highestPrice or stats[5] or 0
        local lowestPrice = stats.lowestPrice or stats[6] or 0
